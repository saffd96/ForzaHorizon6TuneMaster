...
**Что сделано (v1 — коммит `d4446d5` «add sliders»):**
- [x] `TuningConstraints.AeroBalance` (−1…+1) — «Grip» ↔ «Speed», смещает `AeroCalculator` (прижимная сила vs сопротивление/максималка)
- [x] `TuningConstraints.ChassisRotation` (−1…+1) — «Stable» ↔ «Agile» = **баланс недостаточной/избыточной поворачиваемости**; один параметр толкает сразу `DifferentialCalculator` (вкл. развесовку AWD-диффа), `SuspensionCalculator` (ARB, пружины, демпферы, клиренс/rake), `TireCalculator` (давление), `BrakeCalculator` (тормозной баланс), `AlignmentCalculator` (камбер/схождение/кастер)
- [x] Каждый слайдер — смещение ВНУТРИ уже существующих физических диапазонов/клампов, не переопределение расчёта
- [x] Единая панель «Стиль вождения» в `TuneResultView.xaml` (два слайдера рядом, с тултипами-пояснениями), а не по контролу на вкладку калькулятора
- [x] Сохраняется в профиль машины — `TuningConstraints` сериализуется целиком в `StorageService`, доп. код не потребовался

**Осталось (v2):**
- [ ] **Gearing Philosophy** — «Acceleration» ↔ «Top Speed»: смещает разброс передач/главную пару (`GearingCalculator`) — третья ось, отложена, пока не устоялась только что переписанная физика передач
- [ ] Показать пользователю эффект в конкретных цифрах при движении ползунка (сейчас есть только текстовый tooltip-хинт, без live-значений % блокировки/ARB/камбера и т.п.)
- [ ] Решить: значения слайдеров — одни на весь профиль машины или отдельно под каждую дисциплину (сейчас общие на профиль; калькуляторы всё ещё считают per-discipline)
- [ ] Совместимость с ручным override: если пользователь вручную поправит конкретное число (например ARB) после слайдера — оно должно «прилипать», не переписываться слайдером при следующем пересчёте (по аналогии с `AllowGearCalculation`) — пока не реализовано
...
| 16 | "Driving Style" panel — Aero Balance + Chassis Rotation (understeer/oversteer: diff+ARB+brake bias+camber+tires) done; Gearing Philosophy pending | P4 — UX | ✅ v1 |
