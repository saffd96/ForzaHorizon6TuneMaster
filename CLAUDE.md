...
| Spring-rate display had a stray ×10 | `Converters.cs` `UnitValueConverter`, `MainViewModel.SpringFrontMinDisplay` | The N/mm and kgf/mm branches multiplied the already-canonical N/mm value by an extra 10× (lbs/in branch didn't); DB spring rates cap at 200 N/mm game-wide, so cars near their ceiling displayed as ~2000 N/mm, looking hardcoded. Fixed 2026-07 by dropping the stray `* 10.0`/`/ 10.0`. `SpringFrontMinDisplay` is currently unbound in any XAML (dead code) but was fixed for consistency since it had the identical bug. |
