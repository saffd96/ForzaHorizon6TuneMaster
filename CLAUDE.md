...
| Wheel-style weight delta uses `MassLevel` (tier) | `Fh6DatabaseService`, `SelectedParts.WheelTierMassDiff` | Formula: tier×D²×W×4.99e-5×0.5 per axle, relative to stock tier. Refitted 2026-07 against 4 real in-game readings on an FXXK Evo at two different wheel fitment sizes (confirms the D²×W shape itself, not just the constant — residual ≤0.6kg at both sizes) — the old 6.8e-5 overshot by 24-34%. `d`/`w` read the CURRENTLY selected rim diameter/tire width (not stock), so this term already prices in the size change together with the style change for that axle. **Bug fixed 2026-07: `ComputeTotalMass` was ALSO adding the flat `TireWidthMassCoef`/`RimSizeMassCoef` terms for the same axle**, double-counting the size portion whenever a non-stock wheel style was combined with a resized rim/tire (the normal way players build a fitment) — this was the confirmed cause of user reports that wheel-style changes weigh much more than real. Fixed: the flat terms now only apply when that axle's wheel style is still stock (`_wheelFrontId`/`_wheelRearId == null`). Also fixed: `_stockRimTier` used to default to 0 when the DB has no `IsStock` `List_Wheels` row for the car's `MediaName` (12/651 cars, several real playable cars — VW Golf R, Maserati MC20, Acura Integra, BMW M2 among them) — now stays `null` and `WheelTierMassDiff` skips the term entirely rather than guessing a wrong baseline. `List_Wheels.Mass` (the raw per-wheel-style kg field) is NOT what drives this — same-tier wheels with different `Mass` (e.g. XD9 Mass=30 vs stock Mass=38, both MassLevel=4) show **zero** weight change in-game, confirming only the tier comparison matters, not the raw mass. Note: tire aspect-ratio changes carry a real in-game mass cost with no corresponding DB column anywhere (`List_UpgradeCarBodyTireAspectRatioFront/Rear` has no MassDiff field) — unmodeled, ~2-6kg per change observed, not fixable from available data |
| Tire width `MassDiff` (DB) undershoots real growth by ~2.6x | `SelectedParts.TireWidthMassCoef` | **Correction (2026-07) of an earlier, wrong "matches within rounding" claim.** Isolating width alone (Toyota GT86 and Ferrari FXXK Evo WP both expose only ONE `TireAspectRatioFront/Rear` row in the DB — profile is a fixed function of width, not an independently selectable option, so no confound from a separate aspect-ratio effect) across 9 real in-game readings, `List_UpgradeCarBodyTireWidthFront/Rear.MassDiff` consistently undershoots by ~2.6x (least-squares fit through origin: k=2.596). Applied as `TireWidthMassCoef=2.6` in `SelectedParts.ComputeTotalMass`, only when that axle's wheel style is stock (see wheel-style row above — otherwise double-counts with `WheelTierMassDiff`). Rim **diameter** (`RimSize.MassDiff`) is a separate, unrelated table/effect — do not conflate; see below, it does NOT get this same correction (inconsistent across cars, see next row). **Known unresolved gap (2026-07 audit):** both `List_UpgradeCarBodyTireWidthFront/Rear.MassDiff` and `List_UpgradeRimSizeFront/Rear.MassDiff` are near-flat, size-independent linear constants across ~93-98% of the whole car roster (0.054 kg/mm width, 1.36 kg/in diameter — see next row) — but the 2.6x/1.10x calibration above was fit only from cars with 17-24"/215-395mm stock wheels. 172/651 cars (26% of the roster) have stock wheel diameter ≤15" and are outside that calibration range; the DB's flat-constant assumption is known to break down there (the already-documented Sprinter FE 13" 2x-diameter outlier below is the sharpest confirmed case, ~4x undershoot on width-equivalent math) — this is very likely still under-predicting tire/rim mass on small-wheeled cars generally, not just that one car. Not fixed in code: doing so responsibly needs new real in-game mass readings from several small-wheeled (≤15") cars to refit, which nobody has collected yet |
| Rim diameter `MassDiff` (DB) undershoots real growth by ~1.10x | `SelectedParts.RimSizeMassCoef` | Early 1-2-step isolated-diameter tests on 3 cars (Sprinter, GT86, FXXK Evo) gave wildly inconsistent ratios (0.7x-2.2x) — turned out to be whole-kg display rounding dominating such small deltas, not a real absence of pattern. Resolved 2026-07 with an 8-step test on a Toyota Tacoma TRD Pro (16" stock, all 8 rim-size levels 16"->24", front+rear together, profile held stock): cumulative deltas up to +25 kg give a tight, consistent fit (ratio 0.92-1.15 across all 8 points). Combined least-squares fit through the origin (Tacoma's 8 points + GT86's 2 + FXXK Evo's 2, all on the stock-tier wheel so `WheelTierMassDiff` contributes nothing): k=1.10. Applied as `RimSizeMassCoef=1.10` in `SelectedParts.ComputeTotalMass`, only when that axle's wheel style is stock (see wheel-style row above). **One car remains a confirmed outlier**: 1985 Toyota Sprinter FE (ordinal 4162, stock fitment 13"), isolated 13"->14" (both axles, profile held stock) gives +6.0 kg real vs +3.0 kg predicted with k=1.10 — 2x over, while the other 3 cars agree with k=1.10 within rounding. Not accommodated in code (per-car workaround would contradict 3-car consensus). **2026-07 audit confirmed this generalizes**: `List_UpgradeRimSizeFront/Rear.MassDiff` is *exactly* 1.36 kg/inch-step for 573/584 cars (98%) regardless of car size — a single global constant, not a per-car fit — so the Sprinter isn't a one-off, it's the sharpest visible case of a systematic small-stock-wheel (≤15", 172/651 cars) blind spot in the calibration range (see tire-width row above); needs new real readings from small-wheeled cars to fix properly |
| Body-kit's own mass delta lives in per-body `InitialMass`, not its `MassDiff` | `SelectedParts.BodyKitCurbWeightDiff` | `List_UpgradeCarBody.MassDiff` is ~always 0; the real kit weight is `List_UpgradeCarBodyWeight`'s stock `InitialMass` for the kit's CarBodyID minus the stock body's (can differ 100+ kg, e.g. a stripped race shell). The body-scoped hood (`List_UpgradeCarBodyHood`) is a separate, additive delta (carbon-hood choice), not the kit's own mass source. **Bug fixed 2026-07: this term silently returned 0 for every car loaded from a saved profile that has a non-stock body kit.** `SetCarData` used to set both `CarBodyOrdinal` and `_stockCarBodyId` from `car.CarBodyId` — but `MainViewModel.LoadSubViewModels` calls `SetCarData(car, resetParts:false)` *before* it patches `car.CarBodyId` to the saved kit's id (that patch needs `Parts.BodyKitPartId`, already loaded, resolved first), so at `SetCarData` time `car.CarBodyId` was still the stock id. Both fields ended up equal to stock, so `BodyKitCurbWeightDiff()`'s `currentBodyId == _stockCarBodyId` short-circuit fired and returned 0 — undercounting real in-game mass by the kit's own weight delta (confirmed ~30 kg on a real profile: calc 872.7 kg vs real 843 kg, resolved to 842.65 kg after the fix). Live editing (picking a kit from the dropdown mid-session) never hit this, since `OnBodyKitChanged` keeps `CarBodyOrdinal` correct without another `SetCarData` call in between — only profile *loading* did. Fixed: `_stockCarBodyId` is now derived directly as `car.CarDbId * 1000` (matches `BuildCarCard`/`PopulateCarFromDb`'s own stock-body convention, independent of `car.CarBodyId`'s patch timing), and when `resetParts` is false, `CarBodyOrdinal` is re-derived from `BodyKitPartId` via `GetCarBodyKitById` instead of trusted from `car.CarBodyId` or the raw deserialized field (a profile saved while this bug was live could have this field's *own* persisted value wrong too — self-perpetuating once introduced by a load→edit→save cycle) |
| Turbo `MassDiff` sometimes identical across adjacent levels | `List_UpgradeEngineTurboSingle` (and likely Twin/CSC/DSC) | Spot-checked on engine 215 (1985 Sprinter FE's Rotary swap): Level 3 and Level 4 single-turbo rows have byte-identical `MassDiff=12.0` **and** `MaxScale=2.5` — i.e. the DB's last tier adds zero extra recorded mass (or boost). One real in-game reading on this exact Level-4 part shows +16 kg total from NA, not the DB's +12 kg — a ~4 kg undercount. Single data point on one engine — not enough to establish whether this is a general "last tier is a data-entry duplicate" pattern worth a code fix, or a one-off; flagged here in case more engines show the same Level-N/N+1 duplication when audited |
